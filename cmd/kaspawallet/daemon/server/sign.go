...
func (s *server) Sign(_ context.Context, request *pb.SignRequest) (*pb.SignResponse, error) {
	...
	signedTransactions, err := s.signTransactions(request.UnsignedTransactions, request.Password, request.PassPhrase)
	if err != nil {
		return nil, err
	}
...
func (s *server) signTransactions(unsignedTransactions [][]byte, password string, passPhrase string) ([][]byte, error) {
	...
	for i, unsignedTransaction := range unsignedTransactions {
		signedTransaction, err := libkaspawallet.Sign(s.params, mnemonics, passPhrase, unsignedTransaction, s.keysFile.ECDSA)
		if err != nil {
			return nil, err
		}
