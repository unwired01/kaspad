	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libkaspawallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], "dkslnkfs7878hFG!", true)
				if err != nil {
			...
			signedTxStep1, err := libkaspawallet.Sign(params, mnemonics[:1], "dkslnkfs7878hFG!", unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}
			...
			signedTxStep2, err := libkaspawallet.Sign(params, mnemonics[1:2], "dkslnkfs7878hFG!", signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}
			...
			signedTxOneStep, err := libkaspawallet.Sign(params, mnemonics[:2], "dkslnkfs7878hFG!", unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libkaspawallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], "nckasd7asdkhnD8Y8sdjashds8y", false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := libkaspawallet.Sign(params, mnemonics, "nckasd7asdkhnD8Y8sdjashds8y", unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}
...
func TestSignPassPhrase(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		params := &consensusConfig.Params
		cfg := *consensusConfig
		cfg.BlockCoinbaseMaturity = 0
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = libkaspawallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], "fDSFA45435FSDCXZCsadase43", false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		signedTxWithLargeInputAmount, err := libkaspawallet.Sign(params, mnemonics, "fDSFA45435FSDCXZCsadase43", unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}
		...
		signedTxWithLargeInputAndOutputAmount, err := libkaspawallet.Sign(params, mnemonics, "fDSFA45435FSDCXZCsadase43", unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}
